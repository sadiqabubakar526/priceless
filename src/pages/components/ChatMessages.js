...
const firebaseRef = firebase.database().ref("Sauq Engine/Profiles");
const firebaseChatRef = firebase.database().ref("Sauq Engine/User-Garbage");
let firebaseChatDBListener = firebase.database().ref("Sauq Engine/Null");
...
function ChatMessages(props) {
    ...
    useEffect(() => {
        ...
        firebaseRef.child(theirUID).once("value").then(theirProfileInfo => {
            ...
            firebase.database().ref("Sauq Engine/User-Garbage").child(chatServer).child(theirUID).on("child_changed", snap => {
                setTextSpoiler({
    ...
    function GetImage(imagePath) {
        const storageRef = firebase.storage().ref("Sauq/User-Shit/" + imagePath.pathName);
